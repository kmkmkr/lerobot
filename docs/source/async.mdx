...
If you want to run async inference with LeKiwi, install the LeKiwi extras as well:

```shell
pip install -e ".[async,lekiwi]"
```
...
For LeKiwi, use `--robot.type=lekiwi` (onboard mode) or `--robot.type=lekiwi_client` (when controlling a remote `lekiwi_host`).
