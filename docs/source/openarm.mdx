...
## OpenArm v1 Motor-Zero Calibration

OpenArm v1 and v1.1 use the zero stored in each Damiao motor as their joint coordinate frame.
Stop every controller, support the arm safely, and run the
[official OpenArm zero-position calibration](https://docs.openarm.dev/software/setup/motor-config/)
for one arm at a time:
...
The procedure moves joints against their mechanical references. Verify the CAN port and arm side
before running it. LeRobot treats the resulting `openarm_v1_motor_zero` angles (in degrees) as the
canonical coordinate frame. It never rewrites motor zero during `connect()`.

<Tip warning={true}>
  Do not run `lerobot-calibrate` for a full-size OpenArm v1 follower or leader. LeRobot rejects that
  command because setting an arbitrary current pose to zero makes collected data, joint limits, and
  deployment commands use different coordinates. Legacy LeRobot OpenArm calibration JSON files are
  ignored.
</Tip>
...
| `coordinate_frame`    | `openarm_v1_motor_zero` | Official OpenArm v1 motor-zero frame (degrees) |
| `joint_limits`        | Side-specific | Conservative limits, validated inside v1 physical limits |
