...
If your LeKiwi has no SO arm mounted, set `--robot.has_arm=false`. In that mode, setup only asks for the wheel motors, and wheel IDs stay the same (`7, 8, 9`).
...
For a base-only LeKiwi (no SO arm), run the same command with `--robot.has_arm=false`. This skips arm motion steps and saves a base-only calibration file.
...
For a base-only setup, add `--robot.has_arm=false` to the host command, and set `has_arm=False` in your `LeKiwiClientConfig` so the action/observation spaces match.
...
To run asynchronous inference with LeKiwi, install both extras on the machine running the async client:

```bash
pip install -e ".[lekiwi,async]"
```

Start the policy server (same machine or remote GPU machine):
...
Then run the async robot client on the machine connected to the LeKiwi motors and cameras:
...
python -m lerobot.async_inference.robot_client \
    ...
    --robot.cameras="{ front: {type: opencv, index_or_path: /dev/video0, width: 640, height: 480, fps: 30}, wrist: {type: opencv, index_or_path: /dev/video2, width: 480, height: 640, fps: 30}}" \
    --task="dummy" \
    --policy_type=smolvla \
...
For OmniVLA-based navigation on LeKiwi, use `--policy_type=omnivla` and your OmniVLA checkpoint path.
The policy converts OmniVLA waypoint outputs with the same PD controller logic as `run_omnivla.py`, then maps to LeKiwi base action keys as:
`{"x.vel": linear_vel_value_limit, "y.vel": 0, "theta.vel": angular_vel_value_limit}`.
This conversion is controlled by `policy.convert_to_lekiwi_action` (enabled by default).

If you are using the networked LeKiwi host/client stack, use `--robot.type=lekiwi_client` instead, and provide `--robot.remote_ip`, `--robot.port_zmq_cmd`, and `--robot.port_zmq_observations`.
